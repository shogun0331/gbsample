...
namespace GB
{
    public class SoundManager : AutoSingleton<SoundManager>
    {
        ...
        public static void Play(RES_AUDIO id, float volume = 1)
        {
            I.Load();
            
            var audioClip = ResManager.Get(id);
            I._EffAudioSource.PlayOneShot(audioClip,volume);
        }

        public static void PlayBG(RES_AUDIO id,float volume = 1,  bool isLoop = true)
        {
