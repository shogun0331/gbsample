...
namespace GB
{
    public class ObjectPooling : AutoSingleton<ObjectPooling>
    {
        ...
        bool Check(RES_PREFAB prefab,int startSize)
        {
            var obj  = ResManager.Get(prefab);
            if(obj == null) return false;

             var t = obj.GetComponent<PoolingType>();
            if(t == null) t = obj.AddComponent<PoolingType>();
                _dictPooling[prefab.path] = new GameObjectPool<PoolingType>(transform,t,startSize);
        ...
        public static GameObject Create(RES_PREFAB prefab,int startSize = 10)
        {
            I.Init();
            if(!I._dictPooling.ContainsKey(prefab.path)) 
            {
                if(I.Check(prefab,startSize) == false)
                return null;
            }
            var o = I._dictPooling[prefab.path];
            var g = o.Rent();
            g.Name = prefab.path;
            g.IsActive = true;
            g.transform.SetParent(null);
